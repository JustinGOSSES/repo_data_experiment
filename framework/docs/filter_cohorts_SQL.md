...
```js
// display(text)
const dbRepos_Filtered = dbRepos.sql`SELECT * FROM reposSQL WHERE size > ${sizeMin} AND stargazers_count > ${stargazer_count_min} LIMIT ${limitNumberRowsToShow}`

```

```js
display(dbRepos_Filtered.length)
//display(dbRepos_Filtered)

const table_SQLunfiltered = 
  view(Inputs.table(dbRepos_Filtered))
